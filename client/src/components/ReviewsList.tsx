...
const ReviewsList: React.FC<ReviewsListProps> = ({ bookId, currentUserReview, reviews, refetch }) => {
  ...
  return (
    <div className="bg-gray-300 w-full max-w-4xl rounded-lg">
      ...
      {addReviewMode
        ...
        <>
          {currentUserReview && (
            <div key={currentUserReview.createdAt} className="p-4 border-t border-gray-400">
              <div className="flex items-center gap-2">
                <p><i>Reviewed by {currentUserReview.userId.username}</i></p>
                <ReviewStars rating={currentUserReview.rating} fixed />
              </div>
              <p className="text-gray-800 font-semibold py-4 text-xl">{currentUserReview.title}</p>
              <p>{currentUserReview.comment}</p>
            </div>
          )}
          {reviews.map((review) => review._id !== currentUserReview?._id ? (
